...
def ensure_xircuits_initialized():
    ...
    if os.environ.get("XIRCUITS_INIT"):
        ...
        if answer in ["y", "yes", ""]:
            ...
            print(
                "Xircuits is not initialized. Please run 'xircuits init' to initialize manually.")
            return None
...
def cmd_start_xircuits(args, extra_args=[]):
    ...
    if extra_args:
        try:
            launch_cmd = "jupyter lab --ContentsManager.allow_hidden=True " + \
                " ".join(extra_args) + " " + news_url_option
            os.system(launch_cmd)
        except Exception as e:
            print("Error in launch args! Error log:\n", e)
    else:
        os.system(
            f"jupyter lab --ContentsManager.allow_hidden=True {news_url_option}")
...
def cmd_compile(args, extra_args=[]):
    ...
    if args.recursive:
        ...
        if args.out_file:
            compile(args.source_file, args.out_file,
                    component_python_paths=component_paths)
        else:
            output_filename = args.source_file.replace('.xircuits', '.py')
            compile(args.source_file, output_filename,
                    component_python_paths=component_paths)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Xircuits Command Line Interface', add_help=False)
    subparsers = parser.add_subparsers(dest="command")

    # 'init' command.
    init_parser = subparsers.add_parser(
        'init', help='Initialize Xircuits in the current directory.')
    init_parser.set_defaults(func=cmd_init)

    # 'start' command.
    start_parser = subparsers.add_parser('start', help='Start Xircuits.')
    start_parser.add_argument(
        'extra_args', nargs='*', help='Additional arguments for Xircuits launch command')
    start_parser.set_defaults(func=cmd_start_xircuits)

    # 'install' command.
    install_parser = subparsers.add_parser(
        'install', help='Fetch and install a library for Xircuits.')
    install_parser.add_argument(
        'library_name', type=str, help='Name of the library to install')
    install_parser.set_defaults(func=cmd_install_library)

    # 'fetch-only' command.
    fetch_parser = subparsers.add_parser(
        'fetch-only', help='Fetch a library for Xircuits. Does not install.')
    ...
    examples_parser = subparsers.add_parser(
        'examples', help='Get example workflows for Xircuits.')
    examples_parser.add_argument('--branch', nargs='?', default="master",
                                 help='Load example workflows to current working directory')
    examples_parser.set_defaults(func=cmd_download_examples)

    # 'compile' command.
    compile_parser = subparsers.add_parser(
        'compile', help='Compile a Xircuits workflow file.')
    compile_parser.add_argument(
        'source_file', type=str, help='Source Xircuits file to compile.')
    compile_parser.add_argument(
        'out_file', nargs='?', type=str, help='Output Python file.')
    ...
    run_parser = subparsers.add_parser(
        'run', help='Compile and run a Xircuits workflow file.')
    run_parser.add_argument(
        'source_file', type=str, help='Source Xircuits file to compile and run (or a Python file to run directly).')
    run_parser.add_argument('out_file', nargs='?',
                            type=str, help='Optional output Python file.')
