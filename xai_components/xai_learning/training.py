                ...
                for folder in tqdm(folders):
                    for file in os.listdir(folder):
                        ...
                        try:
                            ...
                            print(
                                f"Error reading file: {os.path.abspath(file)}. Skipping..."
                            )
...
class TrainTestSplit(Component):
    ...
    def execute(self, ctx) -> None:
        ...
        splits = train_test_split(
            self.dataset.value[0],
            self.dataset.value[1],
...
class EvaluateAccuracy(Component):
    ...
    def execute(self, ctx) -> None:
        (loss, acc) = self.model.value.evaluate(
            self.eval_dataset.value[0], self.eval_dataset.value[1], verbose=0
        )
...
class SaveKerasModel(Component):
    ...
    def execute(self, ctx) -> None:
        ...
        model_name = (
            ...
            else os.path.splitext(sys.argv[0])[0] + ".h5"
        )
