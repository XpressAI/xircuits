...
class CreateUnetModel(Component):
    ...
    def __init__(self):
        # self.train_loader = InArg.empty()
        # self.test_loader = InArg.empty()
        self.done = False
        ...
        # self.no_epochs = InArg.empty()
        # self.write_model_path = InArg.empty()
    ...
    def execute(self) -> None:
        if torch.cuda.is_available():
            if torch.cuda.device_count() > self.gpu.value:
                device_name = f'cuda:{self.gpu.value}'
            else:
                device_name = 'cuda:0'
        else:
            device_name = 'cpu'
        ...
        early_stopping = EarlyStopping(patience=self.earlystop.value, verbose=self.verbose.value, delta=0, n_epoch=self.no_epochs.value)
...
class ImageTrainTestSplit(Component):
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            train_set_raw, val_set_raw = torch.utils.data.random_split(dataset_raw, [train_set_count, test_set_count],generator=torch.Generator().manual_seed(self.random_seed.value))

            dataset_mask = torchvision.datasets.ImageFolder(Path(self.input_str.value[1]), transform = TRANSFORM_IMG)
            train_set_mask, val_set_mask = torch.utils.data.random_split(dataset_mask, [train_set_count, test_set_count],generator=torch.Generator().manual_seed(self.random_seed.value))
...
class PrepareUnetDataLoader(Component):
    ...
    def execute(self) -> None:        
        ...
        train_loader = torch.utils.data.DataLoader(train_set,
                                                   ...
                                                   pin_memory=self.pin_memory.value
                                                   )
