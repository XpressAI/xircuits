...
(function(root, mod) {
  ...
  var indentHist = exports.indentHist = {
    ...
    count: function () { return indent.length; },
    len: function (i) {
      if (typeof i === 'undefined' || i >= indent.length) i = indent.length - 1;
      return indent[i].length;
    },
    isIndent: function(s) {
      return indent.length === 0 || s.length > this.len();
    },
    isDedent: function(s) {
      return indent.length > 0 && s.length < this.len();
    },
    ...
    addDedent: function (s) {
      dedentCount = 0;
      for (var i = indent.length - 1; i >= 0 && s.length < indent[i].length; --i)
        ++dedentCount;
    },
    updateDedent: function () { dedentCount = this.count(); },
    pop: function () {
      --dedentCount;
      indent.pop();
    },
  ...
  var scope = exports.scope = {
    ...
    init: function () { 
        namespaces = [{ type: 'g', map: {} }]; 
    },
    current: function(offset) {
      offset = offset || 0;
      return namespaces[namespaces.length - offset - 1];
    },
    startClass: function (id) {
      this.current().map[id] = 'c';
      namespaces.push({ type: 'c', map: {}, className: id });
    },
    startFn: function (id) {
      this.current().map[id] = 'f';
      namespaces.push({ type: 'f', map: {}, fnName: id });
    },
    end: function () { namespaces.pop(); },
    ...
    isUserFunction: function(name) {
      ...
      for (var i = namespaces.length - 1; i >= 0; i--)
        for (var key in namespaces[i].map)
          if (key === name && namespaces[i].map[key] === 'f')
            return true;
      return false;
    },
    ...
    isNewObj: function (id) {
      for (var i = namespaces.length - 1; i >= 0; i--)
        if (namespaces[i].map[id] === 'c') return true;
        else if (namespaces[i].map[id] === 'f') break;
      return false;
    },
