...
import { redoIcon, tableRowsIcon } from '@jupyterlab/ui-components';
...
export class XpipeDebuggerSidebar extends Panel {
  ...
  constructor(options: Breakpoints.IOptions) {
    ...
    /**
     * Create a table visualizer button toolbar item.
     */
     let tableButton = new ToolbarButton({
      icon: tableRowsIcon,
      tooltip: 'Open xpipe visualizer',
      onClick: (): void => {
        app.commands.execute(commandIDs.openTableVisualizer);
      }
    });

    debuggerToolbar.toolbar.insertItem(0, 'xpipe-next-node', nextNodeButton)
    debuggerToolbar.toolbar.insertItem(1, 'xpipe-table-visualizer', tableButton)
